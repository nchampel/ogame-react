...
const Multiverse = (props) => {
  const {
    ...
    setResources,
    planetsMultiverse,
    setPlanetsMultiverse,
    starship,
    setStarship,
    planetsDiscovered,
    setPlanetsDiscovered,
    planetsNotDiscovered,
    setPlanetsNotDiscovered,
    isAuthenticated,
  } = props;
  ...
  const [resourcesRobbed, setResourcesRobbed] = useState({
    metal: 0,
    crystal: 0,
    tritium: 0,
  });
  ...
  const handleResourcesRobbed = (planet, planetsDiscoveredTemp) => {
    ...
    planetsTemp[
      planetsMultiverse.findIndex((element) => element.id === planet.id)
    ].metal = 0;
    planetsTemp[
      planetsMultiverse.findIndex((element) => element.id === planet.id)
    ].crystal = 0;
    ...
    planetsDiscoveredTemp[0].tritium = 0;
    ...
    resourcesRobbedTemp.tritium = planet.tritium;
    ...
    // planetsNotDiscoveredTemp[0].tritium = 0
    // setPlanetsDiscovered(planetsNotDiscoveredTemp)
  };

  const getResultsAttack = useCallback(
    async (planet, planetsDiscoveredTemp) => {
      try {
        const dataResults = await planetsApi.getResultsAttack(
          planet,
          starship,
          resources,
          localStorage.getItem("jwt").replaceAll('"', "")
  ...
  const SaveDiscoveredPlanet = useCallback(async (planetId) => {
    try {
      await planetsApi.SaveDiscoveredPlanet(
        planetId,
        localStorage.getItem("jwt").replaceAll('"', "")
      );
    } catch (err) {
  ...
  const fight = (planet, idx) => {
    ...
    // console.log(planet)
    const planetsDiscoveredTemp = [...planetsDiscovered];
    if (1000000 <= resources.tritium && starship.is_built) {
      if (!planet.is_discovered) {
        ...
        const planetsNotDiscoveredTempUpdated = planetsNotDiscoveredTemp.filter(
        ...
        >
          <Box sx={{ ml: 2 }}>
            <Typography fontSize={12}>
              {!planet.is_discovered || planet.type === "ressources"
                ? "-"
                : planet[level]}
            </Typography>
          </Box>
  ...
  const tableCellRowResources = ["metal", "crystal", "tritium"];
  ...
  return (
    <>
      ...
      <Box sx={{ minHeight: "600px" }}>
        ...
        <Typography>{`Vie : ${starship.life_level} Armes : ${
          starship.fire_level
        } Bouclier : ${starship.shield_level} Vaisseau ${
          starship.is_built ? "opérationnel" : "détruit"
        }`}</Typography>
        <Card>
          <TextField
          ...
          />
          <Button onClick={() => handlePlanetsToDisplay("discovered")}>
            Découvertes
          </Button>
          <Button onClick={() => handlePlanetsToDisplay("notDiscovered")}>
            Non Découvertes
          </Button>
